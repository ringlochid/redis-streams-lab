...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--consumer", default=os.getenv("CONSUMER", "worker-1"), help="Consumer name"
    )
    p.add_argument("--group", default=GROUP, help="Consumer group name")
    p.add_argument(
        "--batch", type=int, default=1, help="How many messages to read at once"
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Base processing delay per message (ms)",
    )
    p.add_argument(
        "--jitter-ms",
        type=int,
        default=0,
        help="Additional random processing jitter (ms)",
    )
    p.add_argument(
...
def read_one(args: argparse.Namespace):
    ...
    fresh = r.xreadgroup(
        args.group, args.consumer, {STREAM: ">"}, count=args.batch, block=args.block_ms
    )
...
def main() -> None:
    ...
    while True:
        ...
        for msg_id, fields in entries:
            ...
            print(f"processing {msg_id} order={order_id} amount={amount}", flush=True)
            ...
            if args.process_ms > 0:
                jitter = (
                    random.uniform(0, args.jitter_ms) if args.jitter_ms > 0 else 0.0
                )
                time.sleep((args.process_ms + jitter) / 1000.0)

            if args.crash_after_side_effect:
                print("simulated crash before XACK", flush=True)
                sys.stdout.flush()
                sys.stderr.flush()
                os._exit(1)

            r.xack(STREAM, args.group, msg_id)
            print(f"acked {msg_id}", flush=True)
